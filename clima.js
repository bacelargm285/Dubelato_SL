...
DB.clima = (function () {
  ...
  /**
   * Cruza clima histórico com vendas diárias.
   * Retorna faixas de temperatura, efeito da chuva, correlação e fatores
  ...
  function analisar(M, diasClima) {
    ...
    for (const [k, v] of Object.entries(vendas)) {
      ...
      pontos.push({ k, data: d, dow: d.getDay(), venda: v, tmax: c.tmax, chuva: c.chuva ?? 0 });
    }
    ...
    // fatores para previsão: dia da semana e faixa de temperatura
    ...
    return { pontos, mediaGeral, porFaixa, mediaChuva, mediaSeco, nChuvosos: chuvosos.length, nSecos: secos.length, r, fatorDow, fatorFaixa, fatorChuva };
  }

  /** Previsão de demanda: média geral × fator do dia da semana × fator da faixa de temperatura (× chuva) */
  function preverDemanda(analise, prev) {
    return prev.map(p => {
      const fx = faixaDe(p.tmax);
      let est = analise.mediaGeral * analise.fatorDow[p.data.getDay()] * (analise.fatorFaixa[fx.id] || 1);
      const chove = (p.probChuva != null && p.probChuva >= 60) || p.chuva >= 5;
      if (chove) est *= analise.fatorChuva;
      return { ...p, faixa: fx, chove, estimativa: est };
    });
  }
